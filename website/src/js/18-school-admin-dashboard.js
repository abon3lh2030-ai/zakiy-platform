  ...
  try {
    ...
    tbody.innerHTML = schoolClassesCache.map(c => `
      <tr>
        <td>
          ${escapeHtml(c.name)}
          <button class="ghost rename-class-btn" data-class-id="${c.id}" data-current="${escapeHtml(c.name)}" style="padding:2px 8px; font-size:12px; margin-inline-start:6px;">✏️ ${t('btn_rename')}</button>
        </td>
    ...
    tbody.querySelectorAll('.rename-class-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        const newName = prompt(t('rename_prompt'), btn.dataset.current);
        if (!newName || !newName.trim() || newName.trim() === btn.dataset.current) return;
        try { await apiCall('PATCH', `/api/school/classes/${btn.dataset.classId}`, { name: newName.trim() }); loadSchoolClasses(); }
        catch (e) { alert(e.message); }
      });
    });
